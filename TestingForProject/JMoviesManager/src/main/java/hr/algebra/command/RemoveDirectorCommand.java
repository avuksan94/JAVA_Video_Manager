...
public class RemoveDirectorCommand implements Command {
    ...
    public RemoveDirectorCommand(Set<DirectorModel> directors, DirectorModel director, MovieModel movie) throws SQLException {
    ...
    public void execute() {
        if (directors.remove(director)) {
            try {
                ...
                Logger.getLogger(RemoveDirectorCommand.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
